...
function ShopService.Init(PlayerDataService)
	...
	MarketplaceService.ProcessReceipt = function(receiptInfo)
		...
		if PlayerDataService.IsReceiptProcessed(data, receiptInfo.PurchaseId) then
			return Enum.ProductPurchaseDecision.PurchaseGranted
		end

		local product = ProductDatabase.GetByProductId(receiptInfo.ProductId)
		if not product then
			PlayerDataService.MarkReceipt(data, receiptInfo.PurchaseId)
			return Enum.ProductPurchaseDecision.PurchaseGranted
		end
		...
		elseif product.category == "Boost" then
			PlayerDataService.ApplyBoost(data, product)
			PlayerDataService.SetAttributes(player, data)
